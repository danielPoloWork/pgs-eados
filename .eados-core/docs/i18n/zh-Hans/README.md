...
- **入站贡献评审**（`/eados review`） —— 按信任层级、策略与风险分流非 owner 的 PR；给出处置建议
  （绝不自动合并）。
- **校准的交互作为治理特性** —— 置信度标注、拒绝谄媚、结构化异议、以证据为先的反驳，均渲染进智能体
  契约并由门禁保持一致；仅为**指令化 + 制品门禁 + 重接地**，从不宣称对实时对话有保证。
- **与供应商无关的模型与投入路由** —— 可在**任何** LLM 下工作。策略只讲能力档位，从不讲模型名；
  带日期的目录分为两层（**提供方**说明每个模型能达到什么，**宿主**说明某个运行时能触及哪些提供方），
  因此单供应商宿主与模型无关宿主都只是一条条目，新增任一方都只是数据。EADOS 通过**环境证据**判断
  自己运行在哪个宿主上 —— 绝不询问智能体自己是什么模型 —— 判断不出来时会明说，并仍然给出档位与
  投入等级。步骤同样参与路由：设计步骤比测试步骤要求更高，而受保护的条目在每个步骤都保持其下限。
  **质量优先、兼顾成本，且是结构性的：***成本只能在已经达到既定下限的模型之间做选择，永远不能拉低
  下限*，因此 token 是真实但严格次要的约束，安全或 ADR 路由不会为省 token 而被悄悄降级。
  全程仅为建议 —— 人类保有模型权威，智能体绝不切换自身会话模型。脚手架生成的仓库获得同一套界面，
  并在不匹配时由检查点告警。
...
**EADOS 会部署或运维我的基础设施吗？** 不会 —— 边界止于受治理的仓库：CI/CD 与发布流水线会被渲染，
部署/监控*文档*也是已规划的范围，但 EADOS 从不置备、部署或运维基础设施
