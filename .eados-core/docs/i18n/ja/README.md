...
[![Release](https://img.shields.io/badge/release-v2.12.0-blue.svg)](https://github.com/danielPoloWork/pgs-eados/releases)
...
打ち出します。ほかのフェーズ（`design`・`plan`・`audit`・`migrate`）はその統治をデリバリー・
...
- [リポジトリ構成](#リポジトリ構成) · [ツール](#ツール) · [EADOS の学びかた](#eados-の学びかた) · [はじめかた](#はじめかた) · [Quickstart](#quickstart)
...
3. **生成はそのうちの一フェーズに過ぎない** —— `design → plan → audit → migrate` が、役割権限と
...
- **ライフサイクル全体を統治** —— 六つのオプトイン・フェーズ（`init · design · plan · scaffold ·
  audit · migrate`）を持続 manifest 上で、各遷移にゲート付き。
...
- **統治機能としての較正された対話** —— 確信度タグ、迎合の排除、構造化された異議、証拠優先の
  押し返し。エージェント契約にレンダリングされゲートで整合性を保つ —— あくまで**指示 + 成果物
  ゲート + 再接地**であり、ライブチャットへの保証を謳うものではありません。
- **プロバイダー非依存のモデル・投入量ルーティング** —— **あらゆる** LLM の下で動作します。
  ポリシーが語るのは能力ティアであってモデル名ではありません。日付付きカタログは 2 階層で、
  **プロバイダー**が各モデルの到達水準を、**ホスト**がそのランタイムから到達できるプロバイダーを
  宣言します。単一ベンダーのホストもモデル非依存のホストも 1 エントリで表せ、どちらの追加もデータ
  だけで済みます。EADOS はどのホストで動いているかを**環境の証拠**から判断します —— エージェントに
  自分が何のモデルかを尋ねることは決してしません —— 判断できないときはそう明言し、それでもティアと
  投入量は提示します。ステップもルーティングされ、設計ステップはテストステップより高く、保護された
  項目はすべてのステップで下限を保ちます。**品質優先・コスト意識、しかも構造として：** *コストは
  すでに獲得済みの下限を満たすモデルの中からしか選べず、下限自体を下げることは決してできません*。
  したがってトークンは実在するが厳密に副次的な制約であり、セキュリティや ADR のルートが節約のために
  黙って格下げされることはありません。全体を通じて助言的 —— モデル権限は人間が保持し、エージェントは
  自らのセッションモデルを切り替えません。スキャフォールドされたリポジトリも同じサーフェスを得て、
  不一致時にはチェックポイントが警告します。
...
（統治された欠陥調査：再現 → 根本原因 → 修正 + 回帰テスト → バグ台帳）、
...
`.eados-core/tools/` 配下はすべて標準ライブラリのみの Python（3.12+）で、`pip install` は不要
です。対話パスでは手動で呼び出すことはありません（エージェントが呼びます）が、これらは確定的な
背骨であり、用途ごとに次のように分類できます：

**Generate —— マニフェストを統治済みリポジトリへ**

| ツール | 何をするか |
|--------|-----------|
| `render.py` | 確定的な Mustache サブセットのレンダラ（マニフェスト → 完全な足場） |
| `render_artifact.py` | 単一テンプレートをレンダリングして配置（単品サンドボックス） |
| `eados.py` | 薄いフェーズオーケストレータ —— 実行可能な `/eados <phase>` |
| `phase_runner.py` | 各フェーズの背後にある状態駆動チェッカ（ゲート + ドメインオーバーレイ） |
| `preflight.py` | パイプラインが前提とするツールチェーンの存在と認証を検証 |
| `seed_milestones.py` | `ROADMAP.md` を読み、GitHub に全マイルストーンを播種 |
| `brownfield.py` · `migration_planner.py` · `sandbox.py` | 既存リポジトリの読取り、移行計画、リファクタ書込みの封じ込め |

**Govern —— 権限・トレーサビリティ・PR・リリース**

| ツール | 何をするか |
|--------|-----------|
| `authority_check.py` | パス→ロールのゲート（誰が何を触れるか） |
| `doctor.py` | `/eados status` —— 一目でわかる健全性の読み取り |
| `traceability.py` · `derive_links.py` | 系譜グラフ + マージ済み PR から導く `roadmap-covers-rfcs` / `traceability-lint` ゲート |
| `risk_score.py` | 監査フェーズの確定的リスクモデル |
| `pr_review.py` · `pr_metadata_check.py` | インバウンド PR 評価 + PR メタデータの存在確認 |
...
| ツール | 何をするか |
|--------|-----------|
| `eados_lint.py` | 工場のセルフリント（18 の一貫性 + カバレッジチェック） |
| `self_review.py` | 生成リポジトリの構造的完全性 |
| `profile_ci_lint.py` | 同梱される各 CI フラグメントが妥当な YAML であること |
| `consistency_lint.py`（リポジトリに同梱） | 汎用の、プロファイル駆動の一貫性チェッカ |
| `yamlmini.py` | 上記すべてが共有する依存ゼロの YAML ローダ |

**Learn —— バージョン管理され人がゲートする記憶ループ**

| ツール | 何をするか |
|--------|-----------|
| `record_run.py` | 機械化された実行記録（ステップ 9）—— 由来から導くオーバーライド + 失敗/評点チャネル |
| `autotune.py` | 蓄積した実行記録からデフォルト変更を提案 |
| `lesson_audit.py` | 教訓に対する回帰・死んだ教訓・低評点の傾向（報告のみ） |
| `lesson_sweep.py` | レビュー時の `Lesson:` フィールドを台帳のドラフトへ収穫 |

*Learn* 群がどのようにループを閉じるかは [EADOS の学びかた](#eados-の学びかた) を参照。

---

## EADOS の学びかた

EADOS はベクトルデータベースも不透明なファインチューニングも用いずに自己改善します。その記憶は
**バージョン管理され、人がゲートし、機械的**です。4 つの成果物、それぞれに明確な書き手 / 承認者 /
強制者があります：

| 記憶 | 何を保持するか | 誰が書くか | 誰が承認するか | 誰が強制するか |
|------|--------------|-----------|--------------|--------------|
| `learning/lessons.yaml` | 恒久的で一般化可能な規則 | エージェントが起草（レビュー時に PR の `Lesson:` フィールド、または `lesson_sweep.py`） | オーナー —— マージ済み PR 本文がそのまま承認 | `eados_lint` の `lessons` |
| `learning/runs/*.yaml` | 生成実行ごとに 1 レコード（オーバーライド・失敗・評点） | `record_run.py` がマニフェストの由来から機械的に | ——（事実、事後編集しない） | `eados_lint` の `run-records` スキーマゲート |
| `autotune.py` · `lesson_audit.py` | レコードから採掘した提案 | ツール（報告のみ） | オーナー | ——（助言） |
| `docs/adr/**` | 荷重を担う設計判断 | エージェントが起草 | オーナー | ADR 索引 + レビュー |

その背骨は**エスカレーション経路**であり、知識は一段ずつ固まります：

> **インシデント → 教訓（助言）→ ゲート（機械的）→ メタゲート（カバレッジ）**

ミスは記録された*教訓*になり、繰り返す教訓（`lesson_audit.py` が浮かび上がらせる）は
`eados_lint.py` の*ゲート*へ昇格して二度と静かには再発せず、**ゲートカバレッジのメタゲート**が
外部から変更されうるすべてのファイル種別をその境界内に収めます。L-0001（「複数行のプレース
ホルダは自らのインデントを携えねばならない」）はすでに全行程を経て —— 今日ではレンダラのロジック
と一つの lint チェックであり、付箋ではありません。
...
**どのモデル？** EADOS はエージェントの推論に依存するため、強力なモデルほど良い結果になります。
**2026-07 時点**では、各ホストに 3 段のラダー（フラッグシップ / バランス / エコノミー）があります：
Claude Code は **Claude Fable 5** · **Opus 5** · **Sonnet 5**、Codex は **GPT Sol** · **GPT Terra** ·
**GPT Luna**。**Gemini 3.5 Flash** も動作します；**Mistral AI** / **Sakana AI** は EADOS では
まだベンチマークしていません。日付き
カタログ [`os/routing/routing.yaml`](../../../orchestrator/os/routing/routing.yaml) が信頼できる
唯一の情報源であり、この段落はそのスナップショットです（両者は同時に更新されます）。
*（モデルの順位は入れ替わりが速いため、恒久的な主張ではなく日付きのスナップショットとして扱ってください。）*
...
- **書き込み制限の生成。** レンダラーと `migrate` サンドボックスは、ターゲットの外へ逃げる書き込み
...
**EADOS は私のインフラをデプロイ・運用しますか？** いいえ —— 境界は統治されたリポジトリまでです。
CI/CD とリリースパイプラインはレンダリングされ、デプロイ/監視の*ドキュメント*も計画済みの領域ですが、
EADOS がインフラをプロビジョニング・デプロイ・運用することはありません
（[ADR-0018](../../adr/0018-ops-delivery-perimeter.md)）。
...
**2026-07 時点**のフラッグシップ段は **Claude Fable 5**（Claude Code）または **GPT Sol**（Codex）。
