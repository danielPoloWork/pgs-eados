...
- **モデル・投入量ルーティングをデータとして** —— 各ロードマップ項目が能力ティア + 投入量を持つ
  （**モデル名ではなくティア**）。スキャフォールドされたリポジトリも同じサーフェスを得ます ——
  `ROADMAP.md` のルーティング凡例（日付付きカタログ・スナップショット）と、インテークのシグナル
  から導出される項目ごとの助言ルート。セッションのモデルがルートから外れるとチェックポイントが
  警告。助言的 —— モデル権限は人間が保持し、エージェントは自らのセッションモデルを切り替えません。
