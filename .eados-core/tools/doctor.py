...
def status_report(manifest, workflow, roadmap_text=None, links=None):
    ...
    # #319: which factory produced this repo. A RECORDED stamp is durable; a derived one is just
    # today's factory and says so, because "rendered by v2.9.0" and "rendered by whatever is
    # checked out right now" are different facts and must not read alike.
    gb = manifest.get("generated_by") if isinstance(manifest, dict) else None
    if isinstance(gb, dict) and str(gb.get("eados_version") or "").strip():
        lines.append(f"generated by: {render.provenance_line(gb)}   (recorded in the manifest)")
    else:
        lines.append(f"generated by: not recorded — record `generated_by:` to make the provenance "
                     f"durable (today's factory is {render.provenance_line(render.factory_provenance())})")
